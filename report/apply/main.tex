...
This section will introduce what is the container, dirty cow, and escape vulnerabilities.
We will focus on the container security part in this research, and the container technique
is a brand new technique implemented from kernel level, therefore, we will also
introduce some vulnerabilities, which is from kernel, to demonstrate how much of an impact
of a container security vulnerability in these days.\\
...
So, when will we use the container?  Well, we often used to run a docker container to host
our services, instance: the servers and some services in the information security club at
NSYSU(National Sun Yat-sen University). However, there are some threats to the container
technique, such as "Dirty CoW" \cite{Dirty_CoW} and "escape vulnerabilities".\\
...
In order to present the container security, we would take the medical system as an example.
...
In these up coming years, the container technique will be applied in many different of area
of the world, especially the medical system or healthcare data exchange systems. Therefore,
we pick the FHIR, the next generation of healthcare data exchange standard, for the case to
study.
...
This section will focus on (\RN{1}) \hyperlink{preliminaries}{preliminaries}, which will
introduce some technical term in this research. (\RN{2}) Some issues are related to the
\hyperlink{security}{containter security}, part. (\RN{3}) \hyperlink{heigh_performance}
{high-performance server}, which will introduce about how is the high performance server
...
We will list four features, 3 terminologies and 2 system calls about the container security
issues.
...
\begin{figure}
  % \centering
  \includegraphics[width=\textwidth]{io_uring.png}
...
\begin{figure}
  % \centering
  \includegraphics[width=\textwidth]{Screenshot from 2021-02-09 19-45-29.png}
